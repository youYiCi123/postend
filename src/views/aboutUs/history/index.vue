  ...
  <div class="app-container">
      ...
      <div style="margin-top: 15px">
        <el-form :inline="true" :model="listQuery" size="default" label-width="140px">
          <el-form-item label="输入搜索：">
            <el-input v-model="listQuery.keyword" class="input-width" placeholder="年份名称" clearable></el-input>
          </el-form-item>
        </el-form>
      </div>
    </el-card>
    ...
    <div class="table-container">
      <el-table ref="roleTable" :data="list" style="width: 100%;" v-loading="listLoading" border>
        <el-table-column label="大事记年份" align="center">
          <template #default="scope">
            <el-tag type="success" style="cursor:pointer;">{{ scope.row.enterpriseYear }}</el-tag></template>
        </el-table-column>
    ...
    <el-dialog :title="isEdit ? '编辑年份' : '添加年份'" v-model="dialogVisible" width="40%">
      <el-form :model="history">
        <el-form-item label="大事记年份：" label-width="150px">
          <el-input v-model="history.enterpriseYear" style="width: 280px"></el-input>
        </el-form-item>
        <el-form-item label="描述：" label-width="150px">
          <el-input v-model="history.yearDesc" type="textarea" :rows="5" style="width: 280px"></el-input>
        </el-form-item>
        <el-form-item label="排序:" label-width="150px">
          <el-input-number v-model="history.sort" :min="1" :max="30" />
        </el-form-item>
      </el-form>
      <template #footer>
...
import { fetchList, createHistory, updateHistory, deleteHistory } from '@/api/history';
...
function handleAdd() {
  ...
  isEdit.value = false;
  history.id = -1,
    history.enterpriseYear = '',
    history.yearDesc = '',
    history.sort = 1
}

function handleDelete(row: History) {
  ElMessageBox.confirm('是否要删除该年份?', '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(() => {
    deleteHistory(row.id).then(response => {
      ElMessage({
        type: 'success',
        message: '删除成功!'
      });
...
function handleUpdate(row: History) {
  dialogVisible.value = true;
  isEdit.value = true;
  history.id = row.id
  history.enterpriseYear = row.enterpriseYear
  history.yearDesc = row.yearDesc
  history.sort = row.sort
}
function handleDialogConfirm() {
  ElMessageBox.confirm('是否要确认?', '提示', {
    ...
    if (isEdit.value) {
      updateHistory(history.id, history).then(response => {
        ElMessage({
          message: '修改成功！',
          type: 'success'
        });
      ...
      createHistory(history).then(response => {
        ElMessage({
          message: '添加成功！',
          type: 'success'
        });
